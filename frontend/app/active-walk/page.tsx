...
import { ActiveWalkAnalysis, ActiveWalkStatus, isActiveWalkCompleted, isActiveWalkProcessing, isActiveWalkStatus, isActiveWalkUnavailableStatus } from "../../lib/active-walk-status";
...
export default function ActiveWalkPage() {
  ...
  const [analysis, setAnalysis] = useState<ActiveWalkAnalysis | null>(null);
  ...
  useEffect(() => {
    if (!analysis || analysis.state !== "processing" || !analysis.analysis_id) return;
    const poll = window.setTimeout(async () => {
      try {
        const next = await request<unknown>(`/active-walk/analyses/${analysis.analysis_id}`, { timeoutMs: 15_000 });
        if (isActiveWalkProcessing(next)) {
          setAnalysis(next);
          return;
        }
        if (isActiveWalkCompleted(next) && isActiveWalkStatus(next.result)) {
  ...
  async function start(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const next = await request<unknown>(`/active-walk/dogs/${dogId}/analyses`, {
        method: "POST",
        body: JSON.stringify({
      ...
      setStarted(false);
      setError(caught instanceof Error ? caught.message : unavailableStatusMessage);
    } finally { if (!pollingStarted) setBusy(false); }
  }
