...
def test_current_environment_uses_fortyguard_heatmap_temperature(monkeypatch):
    calls = []
    monkeypatch.setattr(current_conditions.fortyguard_service, "submit_heatmap", lambda payload, **_: calls.append(("heatmap", payload)) or {"data": {"activity_id": "map"}})
    monkeypatch.setattr(current_conditions.fortyguard_service, "submit_environment", lambda payload, **_: calls.append(("environment", payload)) or {"data": {"activity_id": "env"}})
